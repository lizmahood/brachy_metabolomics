...
class FisherExactTest:
	...
	def print_report (self, k, n, C, G):
		...
		print(f"     Selected   {k} (k)    {n-k}        {n} (n)")
		print(f" Not selected   {C-k}        {G-C-n+k}        {G-n}")
		print(f"        Total   {C} (C)    {G-C}        {G} (G)")
		print()
		print(f"         Enrichment : {self.enrichment(k, n, C, G)}")

		left, right, two_tailed = self.pvalue(k, n, C, G)

		print(f"       Left p-value : {left}")
		print(f"      Right p-value : {right}")
		print(f" Two-tailed p-value : {two_tailed}")
